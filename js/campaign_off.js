...
const DIFFICULTIES = {
  normal: { id: 'normal', name: 'Normalny', hpMul: 1, accMul: 1, dmgMul: 1,
            creditMul: 1, timerMul: 1, pressureMul: 1 },
};
function difficulty() { return DIFFICULTIES.normal; }
...
let lifeStats = (() => {
  ...
  } catch (e) { /* brak / uszkodzone -> od zera */ }
  return { kills: 0, shots: 0, hits: 0 };
})();
let statsDirty = false, statsSavedAt = 0;
...
function statsFlush(force = false) {
  ...
  statsSavedAt = now;
  statsDirty = false;
  try { localStorage.setItem(STATS_KEY, JSON.stringify(lifeStats)); } catch (e) { /* ignore */ }
}
...
function missionHpTrack() { /* medal counter lived in the campaign */ }

/* ---- radio: dialogi były częścią kampanii ---- */
let radioHoldT = 0;
function updateRadio() { /* no queue without missions */ }
...
function renderMenuMeta() {
  el('menu-progress').textContent = game.best
    ? `Rekord areny: ${game.best}` : 'Arena bez końca';
}
