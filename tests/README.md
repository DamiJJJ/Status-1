...
| `menu_test.py` | MENU-1: menu główne z panoramą (boot do `screen-menu`, dryf kamery, zapełniona scena), pełna pętla nawigacji (arena/statystyki/kampania/ustawienia/zbrojownia i powroty), panorama wyłączona w rozgrywce i przy pauzie, powrót po `quitToMenu`, motyw menu z `assets/` (pętla, suwak muzyki, wyciszenie na czas misji, powrót bez przewijania, `file://`) |
